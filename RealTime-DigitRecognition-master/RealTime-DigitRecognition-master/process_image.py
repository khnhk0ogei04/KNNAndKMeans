...
model = load_model('./cnn_model/digitrecognition1.h5')
...
def get_output_image(path):
    ...
    results = cv2.findContours(thresh, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    if len(results) == 3:  # OpenCV 2.x
        im2, contours, hierarchy = results
    else:  # OpenCV 3.x and newer
